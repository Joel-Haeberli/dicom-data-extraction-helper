...
def print_dicom_pixel_array(filepath, convert_to_hu=False, apply_overlay=False, overlay_file=None):
    ...
    if apply_overlay:
        ...
        # Try to get overlay as pixel array (for mask images)
        try:
            overlay_array = overlay_ds.pixel_array
        except AttributeError:
            # Try to get traditional DICOM overlay
            try:
                overlay_array = overlay_ds.overlay_array(0)  # First overlay
            except Exception as e:
                print(f"Error: {overlay_file} does not contain valid pixel data or overlay data. Details: {e}")
                return
